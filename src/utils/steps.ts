...
import { BaseResearchError, isResearchError } from '../types/errors';
import { logger, createStepLogger } from './logging';
...
export function wrapStepWithErrorHandling(
  ...
  const wrappedStep: ResearchStep = {
    ...
    rollback: step.rollback ? async (state: ResearchState): Promise<ResearchState> => {
